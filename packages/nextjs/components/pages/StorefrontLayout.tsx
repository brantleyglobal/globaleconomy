...
export default function HomePageLayout() {
  ...
  const [selectedTokenSymbol, setSelectedTokenSymbol] = useState<string>(supportedTokens[0].symbol);
  ...
  useEffect(() => {
    const resumeStripeFlow = async () => {
      const handled = await handleStripeReturn();
      if (handled) {
        const store = useCheckoutStore.getState();
        store.setField("buyModalOpen", true);
        store.setField("currentStep", 4);
      }
    };
