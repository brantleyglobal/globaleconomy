...
export const XchangeRefundTable = ({ transactions }: { transactions: Transaction[] }) => {
  const { address: connectedAddress } = useAccount();

  // Filter only transactions where refund is true/Truthy
  const refundTransactions = transactions.filter(tx => Boolean(tx.refund));

  if (!refundTransactions.length) {
    return (
      <div className="text-zinc-400 text-sm mt-4">
        No Xchange refunds transactions found.
      </div>
    );
  }
