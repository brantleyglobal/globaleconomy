...
contract Dividend757 is Initializable, ERC20Upgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function initialize(
        ...
        __ERC20_init("GlobalDomnionX", "GBD757"); 
    ...
    function update(uint256 currentQuarter, uint256 month, uint256 ts) public {
        ...
        if (localUnlock == 0) {
            ...
            if(absoluteMonth > 5) {
                adjustedTs = ts.addMonths(1); 
            } else {
                adjustedTs = ts.subMonths(1);
            }

            contractTime = adjustedTs;

            startQuarter = 2026 * 4 + 2;
