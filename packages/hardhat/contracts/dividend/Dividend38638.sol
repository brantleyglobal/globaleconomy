...
contract Dividend38638 is Initializable, ERC20Upgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function initialize(
        ...
        __ERC20_init("GlobalDomnionX", "GBD38638"); 
    ...
    function update(uint256 currentQuarter, uint256 month, uint256 ts) public {
        ...
        if (localUnlock == 0) {
            ...
            if(absoluteMonth > 5) {
                adjustedTs = ts.addMonths(32); 
            } else {
                adjustedTs = ts.subMonths(32);
            }

            contractTime = adjustedTs;

            startQuarter = 2026 * 4 + 12;
