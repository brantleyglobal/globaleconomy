...
contract Dividend48848 is Initializable, ERC20Upgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function initialize(
        ...
        __ERC20_init("GlobalDomnionX", "GBD48848"); 
    ...
    function update(uint256 currentQuarter, uint256 month, uint256 ts) public {
        ...
        if (localUnlock == 0) {
            ...
            if(absoluteMonth > 5) {
                adjustedTs = ts.addMonths(42); 
            } else {
                adjustedTs = ts.subMonths(42);
            }

            contractTime = adjustedTs;

            startQuarter = 2026 * 4 + 15;
