...
contract Dividend252 is Initializable, ERC20Upgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function initialize(
        ...
        __ERC20_init("GlobalDomnionX", "GBD252"); 
    ...
    function update(uint256 currentQuarter, uint256 month, uint256 ts) public {
        ...
        if (localUnlock == 0) {
            ...
            if(absoluteMonth > 5) {
                adjustedTs = ts.addMonths(4); 
            } else {
                adjustedTs = ts.subMonths(4);
            }
