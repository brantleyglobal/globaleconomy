...
contract Dividend13213 is Initializable, ERC20Upgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function initialize(
        ...
        __ERC20_init("GlobalDomnionX", "GBD13213"); 
    ...
    function update(uint256 currentQuarter, uint256 month, uint256 ts) public {
        ...
        if (localUnlock == 0) {
            ...
            if(absoluteMonth > 5) {
                adjustedTs = ts.addMonths(7); 
            } else {
                adjustedTs = ts.subMonths(7);
            }

            contractTime = adjustedTs;

            startQuarter = 2026 * 4 + 4;
