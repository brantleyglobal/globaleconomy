...
contract GlobalDollarT is Initializable, ERC20Upgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function initialize(
        ...
        __ERC20_init("GlobalDollarT", "GBDt"); 
    ...
    function update(uint256 currentQuarter, uint256 month, uint256 ts) public {
        ...
        if (unlockQuarter == 0) {
            ...
            uint256 callQuarter = currentQuarter + committedQuarters + rectifier;
        ...
        if (currentQuarter >= comingQuarter && locked == false) {
            ...
            uint256 key = monthKey;
            uint256 advance = (committedQuarters * 3) + (redeemPeriod * 3);
            monthKey = ((key + advance - 1) % 12) + 1;
